...
description: Anti-generic frontend design skill for landing pages, public SaaS sections, product showcases, and redesigns. Use to improve layout, typography, spacing, motion, responsive behavior, and visual specificity while preserving existing app behavior.
---
...
## AVERA Local Override

This skill must cooperate with the repository's engineering skills.

- It is a frontend taste layer, not a product architecture layer.
- If the UI depends on DB/API/runtime data, use `impact-audit` first.
- If the task needs backend, DB, vendor/admin UI, public UI, tests, or deployment changes together, use `full-stack-feature`.
- If the user asks for code, use `code` after the design read and scope are clear.
- Existing project constraints win: installed dependencies, Tailwind setup, tokens, icon system, routes, SEO, analytics, auth, API contracts, and accessibility.
- Do not install new design systems, fonts, icon sets, animation libraries, GSAP, or Three.js unless the user approves or the project already uses them.
- Lucide is acceptable when the project already uses `lucide-react` or repository rules prefer Lucide.
- Inter or an existing font is acceptable when it is already part of the app. Avoid font churn unless typography is explicitly in scope.
- Product UI mockups are allowed for SaaS landing sections when they are realistic and domain-specific, not generic placeholder rectangles.
- For AVERA/PMS, prefer hospitality SaaS specificity: rooms, reservations, housekeeping, pricing plans, occupancy, revenue, guests, invoices, properties, and vendor workflows.
- For operational admin/vendor/tenant UI, optimize for scanability and repeated work, not decorative landing-page composition.

## Skill Handoff

- Use `redesign-existing-projects` for improving an existing UI section/page.
- Use `high-end-visual-design` when the user asks for premium, quiet luxury, expensive, elegant, or agency-level polish.
- Use `impact-audit` when frontend data or behavior depends on DB/API/config.
- Use `coverage-audit` when auditing many screens, buttons, forms, UI states, or FE/BE field contracts.
- Use `deepdive` when the existing UI flow is unclear.
- Use `bugfinder` if visual implementation causes build, runtime, layout, or responsive failures.
